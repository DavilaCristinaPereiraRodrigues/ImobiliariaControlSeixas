...
public class Login  {
	...
	try {
				...
				//	login.setResizable(false);  //Fazer que a tela não redimensione
	...
	public Login(){
		JFrame janelaLogIn = new JFrame("Control Seixas - Software de Controle Imobiliário");
	...
	//	JFrame janelaLogIn = new JFrame("Janela de LogIn");
		...
		janelaLogIn.setTitle("Control Seixas - Software de Controle Imobiliário");
		...
		btnEntrar.addActionListener(new ActionListener() {
			...
			if(corretor != null){
				...
				else {//Se não abre o outro menu COmum
				...
				//JOptionPane.showMessageDialog(((JButton)arg0.getSource()).getParent(), "Login ou senha inválido.", "Erro", JOptionPane.ERROR_MESSAGE);//Este funciona sem precisar conhecer a referencia do continer/janela do componente que dispara o action
				JOptionPane.showMessageDialog(janelaLogIn, "Login ou senha inválido.", "Erro", JOptionPane.ERROR_MESSAGE);
				txtLogin.setText("");
				txtsenha.setText("");
			}
		...
		*/
		//Imagem de fundo - tem que ser por último para aparecer a imagem
		...
		janelaLogIn.getContentPane().add(label);
				...
				janelaLogIn.setVisible(true); 
				janelaLogIn.setResizable(false);//Fazer que a tela não redimensione
